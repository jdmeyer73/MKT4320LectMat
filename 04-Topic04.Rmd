...
* Plots show predicted DV at different levels of significant IVs 
    ...
    avg6mou <- mean(telecom$avg6mou)
    cc <- mean(telecom$cc)
    da <- mean(telecom$da)
    ovrmou <- mean(telecom$ovrmou)
    income <- mean(telecom$income)
    # Create dataframe for prediction at different levels of 'avg6mou'
    a6m.pred <- crossing(avg6mou=seq(0,1500,100), # 14 levels
                         cc=c(1.40, 1.17), da=da, ovrmou=ovrmou, income=income,
                         own=c("Yes","No"))
    ...
    p1 <- ggplot(aes(x=avg6mou, y=p$fit, color=factor(own)), data=a6m.pred) +
        geom_line(size=1) +
    ...
    cc.pred <- crossing(avg6mou=avg6mou, cc=seq(0,10,1), da=da,
                        ovrmou=ovrmou, income=income,
                        own=c("Yes","No"))
    cc.pred$p <- as.data.frame(predict.lm(target, cc.pred,
                                          interval="confidence"))
    p2 <- ggplot(aes(x=cc, y=p$fit, color=factor(own)), data=cc.pred) +
        geom_line(size=1) +
        geom_ribbon(aes(ymin=p$lwr, ymax=p$upr), alpha=0.2) +
    ...
    da.pred <- crossing(avg6mou=avg6mou, cc=cc, da=seq(0,10,1),
                        ovrmou=ovrmou, income=income,
                        own=c("Yes","No"))
    da.pred$p <- as.data.frame(predict.lm(target, da.pred,
                                          interval="confidence"))
    p3 <- ggplot(aes(x=da, y=p$fit, color=factor(own)), data=da.pred) +
        geom_line(size=1) +
        geom_ribbon(aes(ymin=p$lwr, ymax=p$upr), alpha=0.2) +
    ...
    om.pred <- crossing(avg6mou=avg6mou, cc=cc, da=da,
                        ovrmou=seq(0,300,25), income=income,
                        own=c("Yes","No"))
    om.pred$p <- as.data.frame(predict.lm(target, om.pred,
                                          interval="confidence"))
    p4 <- ggplot(aes(x=ovrmou, y=p$fit, color=factor(own)), data=om.pred) +
        geom_line(size=1) +
        geom_ribbon(aes(ymin=p$lwr, ymax=p$upr), alpha=0.2) +
        labs(x="Mean Monthly Overage Minutes", 
             y="Linear Prediction") +
        theme(legend.position="none")
    prow <- plot_grid(p1, p2, p3, p4, nrow=2)
    prow
