...
``` {r t2packagesdata, message=FALSE}
...
# Want to predict 'sales' for different levels of IVs
...
m.ivs <- advtsales %>%
   ...
   summarise(ad_tv=mean(ad_tv, na.rm=TRUE),
             ad_radio=mean(ad_radio, na.rm=TRUE),
             ad_paper=mean(ad_paper, na.rm=TRUE))

# Second, create a df of values to predict with, using 'merge'
#     'merge' simply merges data frames
tv.pred <- merge(data.frame(ad_tv=seq(0, 300, 25)), # Variable of interest
                  m.ivs[,c("ad_radio", "ad_paper")]) # Mean levels of other variables

# Third, use model results to predict DV for values in 'tv.pred' data frame
...
tv.pred$p <- as.data.frame(
   predict.lm(results,    # Object with model results    
              tv.pred,  # df of IV values to predict with
              interval="confidence"))   # Request CIs

# Last, use 'tv.pred' for margin plot; save plot
p1 <- tv.pred %>%
   ggplot(aes(x=ad_tv,   # Age on x-axis
              y=p$fit)) +   # 'sales' prediction on y-axis
      geom_line(size=.5, color="red4") +   # Draw predicted line
      geom_ribbon(aes(ymin=p$lwr,  # Draws the confidence interval bands
                  ymax=p$upr),
                  alpha=0.2, fill="red4") + # Sets transparency level
      ...
      # Change text size
      theme(text=element_text(size=15)) +    
      labs(x="TV Advertising", y="Linear Prediciton")

# Repeat for other two variables
radio.pred <- merge(data.frame(ad_radio=seq(0, 50, 5)),
                    m.ivs[,c("ad_tv", "ad_paper")])
radio.pred$p <- as.data.frame(predict.lm(results, radio.pred,
                                         interval="confidence"))
p2 <- radio.pred %>%
   ggplot(aes(x=ad_radio, y=p$fit)) +
      geom_line(size=.5, color="forestgreen") +
      ...
      scale_y_continuous(limits=c(5,30), expand=c(.025,.025), 
                         breaks=seq(5,30,5), minor_breaks=NULL) +
      theme(text=element_text(size=15)) +    
      labs(x="Radio Advertising", y="Linear Prediciton")

paper.pred <- merge(data.frame(ad_paper=seq(0, 120, 10)),
                    m.ivs[,c("ad_tv", "ad_radio")])
paper.pred$p <- as.data.frame(predict.lm(results, paper.pred,
                                         interval="confidence"))
p3 <- paper.pred %>%
   ggplot(aes(x=ad_paper, y=p$fit)) +
      geom_line(size=.5, color="darkorange") + 
      geom_ribbon(aes(ymin=p$lwr, ymax=p$upr), 
                  alpha=0.2, fill="darkorange") +
      scale_x_continuous(limits=c(0,120), expand=c(.025,.025), 
                         breaks=seq(0,120,20), minor_breaks=NULL) +
      scale_y_continuous(limits=c(5,30), expand=c(.025,.025), 
                         breaks=seq(5,30,5), minor_breaks=NULL) +
      theme(text=element_text(size=15)) +    
      labs(x="Paper Advertising", y="Linear Prediciton")
