...
``` {r t2results1, echo=FALSE, comment=NA, cache=TRUE}
...
* Use $F\text{-statistic}$ to test
* Conclusion:  With a $F\text{-statistic}=570.3$ and a $p\text{-value}<0.001$, we conclude that at least one $\beta_k$ is significant
...
# Create new data for prediction with 'size' as focus
size.pred <- crossing(size=seq(0,240000,20000), # 11 levels
...
ggplot(aes(x=size,  # levels of 'size' for x-axis
   ...
   # Next two commands scale x and y axes
   scale_x_continuous(breaks=seq(0,240000,40000), expand=expansion(mult=c(.2,.3)), minor_breaks=NULL) +
