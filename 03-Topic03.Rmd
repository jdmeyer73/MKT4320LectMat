...
* Can visually examine how $\Pr(buy)$ changes with a variable for continuous IVs
    * Factor variables can be separate lines (see Figure \@ref(fig:t3mpage01) or separate plots see \@ref(fig:t3mpage02))
    ...
    ggplot(aes(x=age, y=pred), data=train.pred) +
        ...
        labs(x="Age", y="Pr(Buy)", color="Gender", fill="Gender",
             caption="Calculated at mean value of salary")
    ```
    
    ```{r t3mpage02, echo=FALSE, cache=TRUE, fig.cap="Margin Plot for Age (Separate Plots for Gender) ([R code](#figure-reffigt3mpage02))"}
    # Create plot with gender on separate plots
    ggplot(aes(x=age, y=pred), data=train.pred) +
        geom_line(aes(color=gender), size=1) +
        geom_ribbon(aes(ymin=lwr, ymax=upr, 
                        color=gender, fill=gender), alpha=.2) +
        facet_grid(~gender) + # Creates separate plots for each gender
        theme(text=element_text(size=15), 
    ...
    * Can create different lines for specific levels of another continuous IV
    ...
    ```{r t3mpage03, echo=FALSE, cache=TRUE, fig.cap="Margin Plot for Age at Different Levels of Salary ([R code](#figure-reffigt3mpage03))"}
    # Want to predict for different levels of IVs
    # Because we often do this for other continuous variables, we first
    ...
    ```{r t3mpage04, echo=FALSE, cache=TRUE, fig.cap="Margin Plot for Salary at Different Levels of Age ([R code](#figure-reffigt3mpage04))"}
